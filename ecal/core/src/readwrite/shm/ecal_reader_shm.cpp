...
namespace eCAL
{
  ...
  void CSHMReaderLayer::SetConnectionParameter(SReaderLayerPar& par_)
  {
    for (const auto& memfile_name : par_.parameter.layer_par_shm.memory_file_list)
    {
      ...
      if (m_memfile_thread_pool)
      {
        ...
        auto data_callback = [this, topic_info](const char* buf_, size_t len_, long long id_, long long clock_, long long time_, size_t hash_, const eCAL::QoS::Policies& msg_qos_)->size_t
        {
          return OnNewShmFileContent(topic_info, buf_, len_, id_, clock_, time_, hash_, msg_qos_);
        };
  ...
  size_t CSHMReaderLayer::OnNewShmFileContent(const Payload::TopicInfo& topic_info_, const char* buf_, size_t len_, long long id_, long long clock_, long long time_, size_t hash_, const eCAL::QoS::Policies& msg_qos_)
  {
    if (m_subgate)
    {
      if (m_subgate->ApplySample(topic_info_, buf_, len_, id_, clock_, time_, hash_, msg_qos_, tl_ecal_shm))
      {
        return len_;
      }
    }
